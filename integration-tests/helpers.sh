...
put_acquire_uri_with_private_key_and_tunnel_info() {
  local pool=$3
  local auth=${4:-""}
...
put_release_uri_with_private_key_and_tunnel_info() {
  local pool=${4}
  local auth=${5:-""}
...
test_auth_failure() {
  test_failure "$1" 'HTTP return code: 407 Proxy Authentication Required'
}
...
test_failure() {
  ...
  ( echo "$output" | grep "$expected_message" >/dev/null 2>&1 )
  rc=$?
